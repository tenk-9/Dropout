...
public class Dropout extends PApplet {
...
class CatchPlate{
    ...
    private final int _fillColor = color(58, 201, 176, 120);
    private final float _emitColor = 200;
    private float _emitColorDynamic = 0;
    private final int _strokeColor = color(58, 201, 176);
    // phisics var
...
class CreditSphere{
    ...
    private final float GravityAcc = -0.1f;
    private final float AirResistanceCoef = 0.001f;
    // sphere var
    private final float _r, _mass;
    private float _v;
    private PVector _coordinate;
    private int _bodyColor = color(255, 255, 255, 255);
    // prediction circle var
    private final float _predictionStartY = 500;
    private final float _predCircleR = 15, _predCircleDetail = 10;
    private int _predColor = color(255, 0, 0, 20);

    public CreditSphere(float r, float mass, int fillColor){
        _r = r;
        _mass = mass;
        _v = 0;
    ...
    public void update(){
        ...
        // time delta: 1frame
        _coordinate.y += _v * 1.0f + 1/2 * (accel * (1.0f * 1.0f));
        _v += accel * 1.0f;
        // reput
        put(_coordinate);
    }
...
PVector cameraPlace = new PVector(0, (float)MaxX * 1, -(float)MaxZ * 0.95f);
// UI
...
public void draw() {
    ...
    perspective(PI/1.5f, PApplet.parseFloat(width)/PApplet.parseFloat(height), 1, AreaWallY);
...
// collision handler
// ----------------------------------------------------
public boolean collided(CreditSphere credit, CatchPlate plate){
    ...
    private PFont _font;
    private final int _textGreen = color(39, 148, 98);
    private final int _textBlue = color(17, 56, 119);
    private final int _textWhite = color(255, 255, 255);
    private final int _textCyan = color(44, 148, 166);
    
    // finishUI variables
    private final int _endUiFill = color(220, 220, 139, 70);

    // textWindow variables
    private final int _textWindowFill = color(48, 10, 36, 200);
    private final int _textWindowStroke = color(35, 35, 35);
    private final int _textWindowStrokeWeight = 2;
    private final float _textFloatZ = -0.001f;
    private final PVector _textWindowTL = new PVector(-MaxX * 1.5f, -55, MaxZ * 0.8f);
    ...
    private final PVector _restartWindowTL = new PVector(-_restartWindowSize.x / 2, -40, -MaxZ * 0.3f);
    ...
    public void drawFinishUI(float score){
        ...
        pushMatrix();
                ...
                emissive(_textWhite);
                textSize(12);
                text(endMsg, 0, -10, 0.001f);
                textSize(12);
                text(scoreMsg, 0, 35, 0.001f);
                textSize(4);
                text("Press 'R' to restart.", 0, 45, 0.001f);
                emissive(0);
            popMatrix();
    }
...
class PlayArea{
    ...
    float _wallHeight;
    float _wallPeakY = 20;
    int _wallColor = color(31, 31, 31, 180);
    int _edgeColor = color(97, 118, 116, 180);
    int _fogColor = color(215, 215, 215, 10);
    int _Y0Color = color(220, 220, 170);
    ...
    public void put(PVector center) {
        // put playArea @center
        float x, y, z;
        x = center.x;
        y = center.y;
        ...
        if(_activateFog){
            pushMatrix();
                ...
                float stopTp = 0.0001f; // この透明度を達成するまでfogを重ねる
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--stop-color=#cccccc", "Dropout" };
    if (passedArgs != null) {
